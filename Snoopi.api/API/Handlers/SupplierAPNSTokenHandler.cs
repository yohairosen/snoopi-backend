...
{
    public class SupplierAPNSTokenHandler : ApiHandlerBase
    {
        public override void Get(HttpRequest Request, HttpResponse Response, params string[] PathParams)
        {
            ...
            if (IsAuthorizedRequest(Request, Response, true, out AppUserId))
            {
                ...
                using (StreamWriter streamWriter = new StreamWriter(Response.OutputStream))
                {
                    ...
                    {
                        if (Query.New<AppSupplierAPNSToken>()
                            .Where(AppSupplierAPNSToken.Columns.SupplierId, AppUserId)
                            .AND(AppSupplierAPNSToken.Columns.Token, PathParams[0])
                            .LimitRows(1)
                            .GetCount(@"*") > 0)
                        {
                    ...
                    using (JsonTextWriter jsonWriter = new JsonTextWriter(streamWriter))
                    {
                            ...
                            .GetCount(@"*") == 0)
                        {
                            try
                            {
                                ...
                                .Where(AppSupplierAPNSToken.Columns.Token, PathParams[0])
                                .Execute();
                               // Query.New<AppSupplierAPNSToken>().Delete()
                               //.Where(AppSupplierAPNSToken.Columns.SupplierId, AppUserId)
                               //.Execute();
                                Query.New<AppSupplierAPNSToken>()
                                .Insert(AppSupplierAPNSToken.Columns.SupplierId, AppUserId)
                                .Insert(AppSupplierAPNSToken.Columns.Token, PathParams[0])
                                .Execute();
                            }
                            catch { }
                        }
        ...
        public override void Delete(HttpRequest Request, HttpResponse Response, params string[] PathParams)
        {
            ...
            if (IsAuthorizedRequest(Request, Response, true, out AppUserId))
            {
                ...
                using (StreamWriter streamWriter = new StreamWriter(Response.OutputStream))
                {
                    using (JsonTextWriter jsonWriter = new JsonTextWriter(streamWriter))
                    {
                        if (Query.New<AppSupplierAPNSToken>().Delete()
                            .Where(AppSupplierAPNSToken.Columns.Token, PathParams[0])
                            .Execute() > 0)
                        {
                            jsonWriter.WriteStartObject();
